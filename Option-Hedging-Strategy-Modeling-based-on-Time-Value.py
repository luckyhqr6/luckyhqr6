...
option_contract_3 = option_contract_2.drop(['kind', 'name', 'exercise_type'], axis=1)

#### 插入一列，列名为'ttm'，代表存续期，以天为单位表示，
# 并保留存续期大于30天的期权合约

##插入一列，列名为'ttm'
option_contract_3['ttm'] = pd.Series(pd.to_datetime(option_contract_3['maturity_date'])- pd.to_datetime(option_contract_3['list_date']))

##以天为单位表示
option_contract_3['ttm'] = option_contract_3['ttm'].map(lambda x: x.days)
...
ETF_start = pd.read_excel('ETF_start.xlsx')
ETF_end = pd.read_excel('50ETF_End.xlsx')
# ETF_start=price_start[price_start['ts_code']=='510050.SH']
# import tushare as ts
# ts.set_token('fbe180aa0a1b06ef2e64e6f5a2d8f11ca1fab6fc4857e2ccf7e271eb')
# pro=ts.pro_api()
# ETF_start = pro.fund_daily(ts_code='510050.SH', start_date='20150101', end_date='20191231')
# ETF_start['trade_date']=ETF_start['trade_date'].apply(lambda x:int(x))
# ETF_start=ETF_start[ETF_start['trade_date'].isin(start_date)]
# ETF_start.sort_values(by='trade_date',ascending=True,inplace=True)
# ETF_start.rename(columns={'pre_close':'前收盘价','trade_date':'Date','close':'收盘价','change':'涨跌','pct_chg':'涨跌幅','vol':'成交量','amount':'成交额','ts_code':'代码'},inplace=True)
# ETF_start.to_excel('ETF_start.xlsx')
### 二. 构建期权对冲交易策略
...
opt_sell = options[i]
# 第一个交易日的期权价格数据
ETF_start['Date']=pd.to_datetime(ETF_start['Date'],format='%Y%m%d')
...
# 第i个月到期日期权价格
ETF_end['Date']=pd.to_datetime(ETF_end['Date'],format='%Y%m%d')
price_data = price_end[price_end['Date_True'] == (list(ETF_end['Date'])[i])]

# 把到期日标的资产价格插入DataFrame opt_sell中
opt_sell['50ETF_price'] = list(ETF_end['收盘价'])[i]

# 把opt_sell和price_data合并
opt_buy = pd.merge(opt_buy, price_data, on='ts_code', how='inner')

# 计算第一个交易日所出售期权在交易日的收益(对多头而言是收益，对我们（空头）而言为损失)
opt_sell['payoff'] = (opt_sell['call_put'] == 'C') * np.maximum(opt_sell['50ETF_price'] -opt_sell['exercise_price'], 0) + (opt_sell['call_put'] == 'P') * np.maximum( opt_sell['exercise_price'] - opt_sell['50ETF_price'], 0)
...
plt.plot(maturity_date_cleaned[:-1], returns, color='royalblue', marker='o', \
         markersize=9)
fund_retuns = (ETF_end['收盘价'] - ETF_start['收盘价']) / ETF_start['收盘价']
...
for i in range(len(ETF_start) - 1):
    opt_sell = options[i]
    opt_sell['50price'] = ETF_start['收盘价'][i]
    ...
    opt_buy = options[i + 1]
    opt_buy['50price'] = ETF_start['收盘价'][i]
